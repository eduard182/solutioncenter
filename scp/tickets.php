...
$statsMC=$thisstaff->getTicketsMTC($thisstaff->showAssignedOnly(),$thisstaff->getDepts());
...
5. Aprobadas
6. Atrasadas.
7. Mis ordenes de Crowdsourcing
8. Crear nueva orden
*/
//print_r($stats);
//print_r($statsMC);
...
$open_name = _P('queue-name','Open');  /* This is the name of the open ticket queue */
   
$nav->addSubMenu(array('desc' => __('Received').' ('.number_format($statsMC['received']).')',
                       'title'=>__('Received Tickets'),
                       'href'=>'tickets.php?status=received',
                       'iconclass'=>'Tickets'),
                    ($_REQUEST['status']=='received'));

$nav->addSubMenu(array('desc' => __('Analysis').' ('.number_format($statsMC['analysis']).')',
                       'title'=>__('Analysis Tickets'),
                       'href'=>'tickets.php?status=analysis',
                       'iconclass'=>'Tickets'),
                    ($_REQUEST['status']=='analysis'));

$nav->addSubMenu(array('desc' => __('Realization').' ('.number_format($statsMC['realization']).')',
                       'title'=>__('Realization Tickets'),
                       'href'=>'tickets.php?status=realization',
                       'iconclass'=>'Tickets'),
                    ($_REQUEST['status']=='realization'));

$nav->addSubMenu(array('desc' => __('Testing').' ('.number_format($statsMC['testing']).')',
                       'title'=>__('Testing Tickets'),
                       'href'=>'tickets.php?status=testing',
                       'iconclass'=>'Tickets'),
                    ($_REQUEST['status']=='testing'));

$nav->addSubMenu(array('desc' => __('Approved').' ('.number_format($statsMC['approved']).')',
                       'title'=>__('Approved Tickets'),
                       'href'=>'tickets.php?status=approved',
                       'iconclass'=>'Tickets'),
...
if (isset($_SESSION['advsearch'])) {
    ...
    $nav->addSubMenu(array('desc' => __('Search').' ('.number_format($count).')',
                           ...
                           'iconclass'=>'Ticket'),
                        (!$_REQUEST['status'] || $_REQUEST['status']=='search' || $_REQUEST['advanced']));
}
