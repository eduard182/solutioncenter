...
if ($_POST) {
    ...
    case 'mass_process':
        ...
        break;
    case 'addmanager-users': /// Agreagado por Francisco Colmenarez, para hacer Managar a los Usuarios (Clientes) 

        if (!$_POST['ids'] || !is_array($_POST['ids']) || !count($_POST['ids'])) {
            $errors['err'] = sprintf(__('Debe Seleccionar Almenos Un Usuario!!!.'));
            ...
            foreach ($_POST['ids'] as $k=>$v) {
                ...
                if ($org->setManagerUser($v)) $i++;

            }/// Fin del For --------------------------------------
            $num = count($_POST['ids']);
            if ($i && $i == $num)
                $msg = sprintf(__('Proceso Exitoso %s Actualizado'), _N('El Usuario Seleccionado fue', 'Los Usuarios Seleccionados fueron', $num));
            elseif ($i > 0)
                $warn = sprintf(__('%1$d of %2$d %3$s Actualizado'), $i, $num,_N('El Usuario Seleccionado fue', 'Los Usuarios Seleccionados fueron', $num));
            elseif (!$errors['err'])
                $errors['err'] = sprintf(__('No se pudo Cambiar  %s a Actualizado'), _N('El Usuario Seleccionado fue', 'Los Usuarios Seleccionados fueron', $num));
