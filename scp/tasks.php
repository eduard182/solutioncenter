    ...
    if ($task) {
        ...
        case 'postnote': /* Post Internal Note */
            ...
            if(($note=$task->postNote($vars, $errors, $thisstaff))) {
                ...
                //echo "<script>window.location.href='tasks.php?id=".$task->getId()."';</script>";
                header("location:tasks.php?id=".$task->getId()."&msg=".$msg);
...
/*$open_name = _P('queue-name','Open'); ///// This is the name of the open tasks queue 

$nav->addSubMenu(array('desc'=>$open_name.' ('.number_format($stats['open']).')',
                       'title'=>__('Tareas Abiertas'),
...
if ($stats['assigned']) {
    ...
    $nav->addSubMenu(array('desc'=>__('Mis Tareas').' ('.number_format($stats['assigned']).')',
...
if ($stats['closed']) {
    $nav->addSubMenu(array('desc' => __('Cerradas').' ('.number_format($stats['closed']).')',
    ...
    $nav->addSubMenu(array('desc' => __('Recibidas').' ('.number_format($stats['received']).')',
                           'title'=>__('Tareas Recibidas'),
                           'href'=>'tasks.php?status=received',
                           'iconclass'=>'assignedTickets'),
                        ($_REQUEST['status']=='received'));

    $nav->addSubMenu(array('desc' => __('An&aacute;lisis').' ('.number_format($stats['analysis']).')',
                           'title'=>__('Tareas En An&aacute;lisis'),
                           'href'=>'tasks.php?status=analysis',
                           'iconclass'=>'assignedTickets'),
                        ($_REQUEST['status']=='analysis'));

    $nav->addSubMenu(array('desc' => __('Realizaci&oacute;n').' ('.number_format($stats['realization']).')',
                           'title'=>__('Tareas En Realizaci&oacute;n'),
                           'href'=>'tasks.php?status=realization',
                           'iconclass'=>'assignedTickets'),
                        ($_REQUEST['status']=='realization'));

    $nav->addSubMenu(array('desc' => __('Testing').' ('.number_format($stats['testing']).')',
                           'title'=>__('Tareas En Testing'),
                           'href'=>'tasks.php?status=testing',
                           'iconclass'=>'assignedTickets'),
                        ($_REQUEST['status']=='testing'));

    $nav->addSubMenu(array('desc' => __('Aprobadas').' ('.number_format($stats['approved']).')',
                           'title'=>__('Tareas Aprobadas'),
                           'href'=>'tasks.php?status=approved',
                           'iconclass'=>'assignedTickets'),
...
if (isset($_SESSION['advsearch:tasks'])) {
    ...
    $count = $tasks->count();
    $nav->addSubMenu(array('desc' => __('Búsqueda').' ('.number_format($count).')',
                           'title'=>__('Búsqueda avanzada de tareas'),
...
if ($thisstaff->hasPerm(TaskModel::PERM_CREATE, false)) {
    $nav->addSubMenu(array('desc'=>__('Nueva Tarea'),
                           'title'=> __('Abrir Nueva Tarea'),
                           'href'=>'#tasks/add',
...
if($task) {
    $ost->setPageTitle(sprintf(__('Tarea #%s'),$task->getNumber()));
