...
<div class="pull-left">
   <?php
    if ($count) {
        ...
        echo sprintf(__('%s no tiene ninguna Tarea'), $ticket? 'Este ticket' :
                'System');
    }
   ?>
</div>
...
<?php csrf_token(); ?>
 ...
 <table class="list" border="0" cellspacing="1" cellpadding="0" width="100">
    ...
    foreach($tasks as $task) {
        ...
        //$status = $task->isOpen() ? '<strong>open</strong>': 'closed'; comentado por hdandrea 17-01-19
        $status = $task->getTaskStatusName($id_status);
