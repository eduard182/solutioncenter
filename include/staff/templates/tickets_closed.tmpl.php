...
if($_REQUEST['start_cl'] != "" && $_REQUEST['end_cl'] != ""){
	
	
	
	$d1 =  strtotime($_REQUEST['start_cl']);
	$f1 =  date('Y-m-d H:i:s',$d1);
	
	$d2 =  strtotime($_REQUEST['end_cl']);
	$f2 =  date('Y-m-d H:i:s',$d2);
	
	//echo $f1."****".$f2;

	if( $f1 <= $f2){
		$rango = '	AND ( DATE_FORMAT(ost_ticket.lastupdate,"%Y-%m-%d")  BETWEEN "'.$_REQUEST['start_cl'].'" AND "'.$_REQUEST['end_cl'].'")';
	}else{
		$rango = "";
	}
}else{
...
$consulta = db_query('SELECT 
					...
					WHERE (ost_ticket_status.`id`= 8) AND `ost_user`.org_id ='.$org_id.' '.$extra_sql.' '.$rango.';');
	*/
...
<table id="table_org_orders_3" class="list" border="0" cellspacing="1" cellpadding="0" >
<thead>
<tr>
	<th>Orden</th>
	<th>Fecha</th>
...
<div id="basic_search" style="background-color: #f9f9f9;border: 1px solid #DCDCDC;">
    <div style="min-height:25px;">
			...
			<label>
                Fecha desde <input type="text" class="dp input-medium search-query" name="start_cl">
            </label>
            <label>
                Fecha hasta <input type="text" class="dp input-medium search-query" name="end_cl">
            </label>
