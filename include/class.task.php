...
class TaskModel extends VerySimpleModel {
    static $meta = array(
        ...
        'joins' => array(
            ...
            'status' => array(
                'constraint' => array('status_id' => 'TaskStatus.id'),
    ...
    function getTaskStatus(){
        return TaskStatus::getTaskStatus();
    }

    function getTaskStatusName($id=""){

        if ($id=="") $id=$this->getStatusId();

        return TaskStatus::getTaskStatusName($id);
    }
...
class Task extends TaskModel implements RestrictedAccess, Threadable {
    ...
    function setStatus($status, $comments='', &$errors=array()) {
        ...
        $status_id=TaskModel::getStatusId();

        if($status==TaskModel::getStatusId()) return false;



        $sql="UPDATE ".TASK_TABLE." SET status_id={$status} WHERE id={$this->getId()};";
        if (!$rs=db_query($sql)) return false;

        $this->closed = null;

        if ($status=="6") $this->closed = SqlFunction::NOW();
        
        $ecb = function($t,$status,$status_id) { 
            
            $ant_status=TaskModel::getTaskStatusName($status_id);
            $act_status=TaskModel::getTaskStatusName($status);
            
            $t->logEvent('edited',array('task'=>array($ant_status,$act_status))); 
        };
        ...
        if ($ecb) $ecb($this,$status,$status_id);

        if ($comments) {
            $errors = array();
            $this->postNote(array(
                        'note' => $comments,
                        'title' => sprintf(__('Status changed to %s'), TaskModel::getTaskStatusName($status))
                        ),
                    $errors,
                    $thisstaff);
        }
    ...
    static function create($vars=false) {
        ...
        */
        //// MODIFICADO POR FRANCISCO COLMENAREZ -----------------------------------------------------------
        //print_r($vars['internal_formdata']['assignee']->ht['staff_id']);
        if ($vars['internal_formdata']['dept_id']) $task->dept_id = $vars['internal_formdata']['dept_id'];
        if ($vars['duedate']) $task->duedate = $vars['duedate']." ".$vars['duedate:time'];
        if ($vars['internal_formdata']['assignee']->ht['staff_id']) $task->staff_id = $vars['internal_formdata']['assignee']->ht['staff_id'];
        //// FIN MODIFICADO --------------------------------------------------------------------------------
    ...
    static function getStaffStats($staff) {
        ...
        $where='';
        if ($depts=$staff->getDepts()) $where=' OR task.dept_id IN('.implode(',', db_input($depts)).')';


        $sql .= 'UNION SELECT \'received\', count(task.id ) AS tasks '
                .'FROM ' . TASK_TABLE . ' task '
                .'WHERE task.status_id=1 '
                .'AND (task.staff_id='.db_input($staff->getId()).$where.')'

                .'UNION SELECT \'analysis\', count(task.id ) AS tasks '
                .'FROM ' . TASK_TABLE . ' task '
                .'WHERE task.status_id=2 '
                .'AND (task.staff_id='.db_input($staff->getId()).$where.')'

                .'UNION SELECT \'realization\', count(task.id ) AS tasks '
                .'FROM ' . TASK_TABLE . ' task '
                .'WHERE task.status_id=3 '
                .'AND (task.staff_id='.db_input($staff->getId()).$where.')'

                .'UNION SELECT \'testing\', count(task.id ) AS tasks '
                .'FROM ' . TASK_TABLE . ' task '
                .'WHERE task.status_id=4 '
                .'AND (task.staff_id='.db_input($staff->getId()).$where.')'

                .'UNION SELECT \'approved\', count(task.id ) AS tasks '
                .'FROM ' . TASK_TABLE . ' task '
                .'WHERE task.status_id=5 '
                .'AND (task.staff_id='.db_input($staff->getId()).$where.')';
    ...
    /// AGREGADO POR FRANCISCO COLMENAREZ PARA BUSCAR EL NUMERO DE OC
    static function getOcTicket($ticket_id){
       $sql="SELECT t.number, d.subject FROM ost_ticket t,  ost_ticket__cdata d WHERE t.ticket_id=".$ticket_id." AND t.ticket_id=d.ticket_id";
       $res = db_query($sql);
       $ticket="";
       $description="";
        while ($arr=db_fetch_array($res)){
            ...
            $ticket=$arr["number"];
            $description=$arr["subject"];
        }
        
    return array($ticket,$description);
    }/// FIN DE LA FUNCION ------------------------------------------- 

    static function getValidaHHTask($task_id){ /// FUNCION QUE VALIDA LAS HH DEL METODO CROWS PARA QUE LAS TAREAS NO SOBREPACEN LAS HORAS DE LAS OC
        ...
        $sql="SELECT  (CASE  WHEN v.analisises >= v2.analisis THEN (v.analisises-v2.analisis)
                ...
                FROM    (SELECT  tk.ticket_id,
                        ...
                        WHERE   tk.ticket_id in (select object_id from ost_task where id =".$task_id.")
                        ...
                        FROM    ost_task t,
                                ost_task__cdata d
                        WHERE   t.object_id in (select object_id from ost_task where id =".$task_id.")
                                AND t.id=d.task_id
                        ) v2; ";
...
class TaskStatus extends VerySimpleModel {
    static $meta = array(
        'pk' => array('id'),
        'table' => TASK_STATUS_TABLE,
        'joins' => array(
            'task' => array(
                'constraint' => array('id' => 'TaskModel.status_id'),
            ),
        ),
    );
    ...
    static function getTaskStatus(){
        $datos=array();
        $sql="Select  id,name FROM  ".TASK_STATUS_TABLE." order by sort;";
        $res = db_query($sql);
        $i=1;
        while ($st=db_fetch_array($res)) {

            $datos[$i]["id"]=$st["id"];
            $datos[$i]["name"]=$st["name"];
            $i++;
        }
    ...
    static function getTaskStatusName($id){

        $sql="Select name FROM  ".TASK_STATUS_TABLE." where id={$id};";
        $res = db_query($sql);
        
        $d=db_fetch_array($res);


    return $d["name"]?$d["name"]:'Sin Estado';
    }
