    ...
    function previewTicket ($tid) {
        ...
        echo sprintf(
                '<div style="width:500px; padding: 2px 2px 0 5px;">
                 <h2>%s</h2>',Format::htmlchars($ticket->getSubject()));
        ...
        echo '</table>';
	 	 if(!($user = $ticket->getOwner())) Http::response(404, 'Unknown user');
			
		echo '<table style="margin-top: 1em;">';
					foreach ($user->getDynamicData() as $entry) {
						echo '<tr><td colspan="2" style="border-bottom: 1px dotted black"><strong>'.$entry->getForm()->get('title').'</strong></td></tr>';
						foreach ($entry->getAnswers() as $a) {
							if('Internal Notes'!=Format::htmlchars($a->getField()->get('label'))){
								echo '<tr style="vertical-align:top"><td style="width:50%;border-bottom: 1px dotted #ccc">'.Format::htmlchars($a->getField()->get('label')).'</td>'.
										'<td style="border-bottom: 1px dotted #ccc">'.$a->display().'</td>'.
								'</tr>';
							}
						}
					 }
        ...
        echo '<hr style="display:none;">
            <table style="display:none;" border="0" cellspacing="" cellpadding="1" width="100%" class="ticket_info">';
        if($ticket->isOpen()) {
            echo sprintf('
                    <tr>
                        <th width="100" style="display:none;">Assigned To</th>
                        <td style="display:none;">%s</td>
                    </tr>',$ticket->isAssigned()?implode('/', $ticket->getAssignees()):' <span class="faded">&mdash; Unassigned &mdash;</span>');
        }
        echo sprintf(
            '   <tr>
                    <th width="100" style="display:none;">Department</th>
                    <td style="display:none;">%s</td>
                </tr>
                <tr>
                    <th style="display:none;">Help Topic</th>
                    <td style="display:none;">%s</td>
                </tr>
                <tr>
                    <th style="display:none;">From:</th>
                    <td style="display:none;">%s <span class="faded">%s</span></td>
                </tr>',
