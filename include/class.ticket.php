...
implements RestrictedAccess, Threadable {
    ...
    /// AGREGADO POR FCOLMENAREZ ------------------------------
    static function getOCTask($ticket_id){  //// FUNCION QUE DEVUELVE RESUMEN DEL METODO CROW DE LAS TASK ASOCIADAS A LA OC
        $sql="SELECT  c.id,
                    c.label,
                    b.field_id,
                    b.value,
                    t.number as tarea
            FROM    ost_form_entry a,
                    ost_form_entry_values b,
                    ost_form_field c,
                    ost_task t,
                    ost_task__cdata d
            WHERE   t.object_id=".$ticket_id." 
                    AND t.id=d.task_id
                    AND a.object_id = d.task_id
                    AND a.object_type='A'
                    AND a.id=b.entry_id
       ...
       $i=0;
        while ($arr=db_fetch_array($res)){
            ...
            $datos[$i]["label"]=$arr["label"];
            $datos[$i]["value"]=$arr["value"];
            $i++;
        }
    ...
    ///---------------------------------------------------------------------
    static function getOCTaskResumen($ticket_id,$staff_id){  //// FUNCION QUE DEVUELVE SUMATORIA RESUMEN DEL METODO CROW DE LAS TASK ASOCIADAS A LA OC
      $sql="SELECT  tk.ticket_id,
                    tk.number,
                    td.analisises + v.analisis as analisis,
                    td.realizaciones + v.desarrollo as realizaciones,
                    td.testinges + v.testing as testing,
                    td.totales + v.estimacion as estimacion,
                    td.holanalisises /*+ v.holanalisis*/ as holanalisis,
                    td.holrealizacion /*+ v.holdesarrollo*/ as holrealizacion,
                    td.holtesting /*+ v.holtesting*/ as holtesting,
                    td.totalholgura /*+ v.holtotal*/ as totalholgura
            FROM    ost_ticket tk,
                    ost_ticket__cdata td,
                    (SELECT  t.object_id as ticket_id,
                             sum(d.t_analisis) as analisis,
                             sum(d.t_desarrollo) as desarrollo,
                             sum(d.t_testing) as testing,
                             sum(d.t_estimacion) as estimacion /*,
                             sum(d.t_holanalisis) as holanalisis,
                             sum(d.t_holdesarrollo) as holdesarrollo,
                             sum(d.t_holtesting) as holtesting,
                             sum(d.t_holtotal) as holtotal*/
                     FROM    ost_task t,
                             ost_task__cdata d
                     WHERE   t.object_id=".$ticket_id."
                             AND t.id=d.task_id
            ...
            WHERE   tk.ticket_id=".$ticket_id."
                    AND tk.ticket_id=td.ticket_id
                    AND tk.ticket_id=v.ticket_id";

       $res = db_query($sql);
       $datos=array();
       $arr=db_fetch_array($res);
       
       $sql="select staff_id,isadmin FROM ost_staff where staff_id=".$staff_id;
       $res_staff = db_query($sql);
       $arr_staff=db_fetch_array($res_staff);

        if($arr_staff["isadmin"]){

            $datos=array("analisis"=>$arr["analisis"],
                    "realizaciones"=>$arr["realizaciones"],
                    "testing"=>$arr["testing"],                              
                    "estimacion"=>$arr["estimacion"],
                    "holanalisis"=>$arr["holanalisis"],
                    "holrealizacion"=>$arr["holrealizacion"],
                    "holtesting"=>$arr["holtesting"],
                    "totalholgura" =>$arr["totalholgura"]);
        }else{
            $datos=array("analisis"=>$arr["analisis"],
                    "realizaciones"=>$arr["realizaciones"],
                    "testing"=>$arr["testing"],                              
    ...
    ///---------------------------------------------------------------------
    ///---------------------------------------------------------------------
    static function getOCResumen($ticket_id,$staff_id){  //// FUNCION QUE DEVUELVE RESUMEN DE LA OC DEL METODO CROW
        ...
        if($arr_staff["isadmin"]){ 
            $criterio=" AND c.name NOT IN ('subject','message','priority') ";
        }else{
            $criterio=" AND c.name NOT IN ('subject','message','priority','holanalisises','holrealizacion','holtesting','totalholgura') ";
        }
        ...
        $sql="SELECT  f.title,
                    ...
                    c.label,
                    b.field_id,
                    (CASE   WHEN c.name IN ('complejidad','prioridad') 
                            THEN REPLACE(SUBSTRING_INDEX(REPLACE(b.value,'\"',''),':',-1),'}','')
    ...
    ///---------------------------------------------------------------------
    ///---------------------------------------------------------------------
    static function getValidaHHTask($ticket_id){  //// FUNCION QUE VALIDA QUE LAS HH DE LAS TAREAS  NO SOBREPASEN LA OC
        
        $datos=array();
        $sql="SELECT  (CASE  WHEN v.analisises > v2.analisis THEN (v.analisises-v2.analisis)
                                ELSE  -1
                        END ) as analisises,        
                        (CASE  WHEN v.realizaciones > v2.desarrollo THEN (v.realizaciones-v2.desarrollo)
                               ELSE  -1
                        END ) as realizaciones,
                        (CASE  WHEN v.testinges > v2.testing THEN (v.testinges-v2.testing)
                               ELSE  -1
                        END ) as testinges,
                        (CASE  WHEN v.totales > v2.estimacion THEN (v.totales-v2.estimacion)
                               ELSE  -1
                        END ) as totales
                FROM    (SELECT  tk.ticket_id,
                                td.analisises,
                                td.realizaciones,
                                td.testinges,
                                td.totales
                        ...
                        (SELECT t.object_id,
                                IFNULL(sum(d.t_analisis),0) as analisis,
                                IFNULL(sum(d.t_desarrollo),0) as desarrollo,
                                IFNULL(sum(d.t_testing),0) as testing,
                                IFNULL(sum(d.t_estimacion),0) as estimacion 
                        FROM    ost_task t,
                                ost_task__cdata d
                        WHERE   t.object_id=".$ticket_id."
                                AND t.id=d.task_id
        ...
        $res = db_query($sql);
        $oc=db_fetch_array($res);

        $datos["analisises"]=$oc["analisises"];
        $datos["realizaciones"]=$oc["realizaciones"];
        $datos["testinges"]=$oc["testinges"];
        $datos["totales"]=$oc["totales"];
    ...
    ///---------------------------------------------------------------------
    ///---------------------------------------------------------------------
    static function  gethhOC ($ticket_id){ /// FUNCION QUE DEVUELVE LOS VALORES DE LAS HH MT-CROWS PARA QUE LAS HH DE GESTION NO SOBREPASEN LAS HH OC INTERNAS
    ...
    ///---------------------------------------------------------------------
    ///---------------------------------------------------------------------
    static function  get_taskCli ($ticket_id){ /// FUNCION QUE DEVUELVE INFORMACIÓN DE LAS TAREAS EN AL VISTA DE CLIENTES

        $datos=array();
        $sql="SELECT  t.id,
                    t.object_id,
                    t.number,
                    DATE_FORMAT(t.created, '%d/%m/%y %H:%i') as created,
                    (CASE WHEN t.closed IS NULL THEN 'open' ELSE 'closed' END) as estado,
                    td.title,
                    t.dept_id,
                    d.name,
                    t.staff_id,
    ...
    ///---------------------------------------------------------------------
    ///---------------------------------------------------------------------
    static function getStaffStatsMTC ($all,$dept){
        
        $filter="";
        $datos=array();
        if ($all==0){

           $filter.="AND dept_id in (";
           $aux="";
        ...
        $sql="SELECT    (SELECT count(ticket_id) FROM ost_ticket WHERE status_id=6 {$filter} ) as received,
                        (SELECT count(ticket_id) FROM ost_ticket WHERE status_id=1 {$filter} ) as analysis,
                        (SELECT count(ticket_id) FROM ost_ticket WHERE status_id=2 {$filter} ) as realization,
                        (SELECT count(ticket_id) FROM ost_ticket WHERE status_id=3 {$filter} ) as testing,
                        (SELECT count(ticket_id) FROM ost_ticket WHERE status_id=7 {$filter} ) as approved";

        $res = db_query($sql);
        $valor=db_fetch_array($res);

        $datos["received"]=$valor["received"];
        $datos["analysis"]=$valor["analysis"];
        $datos["realization"]=$valor["realization"];
        $datos["testing"]=$valor["testing"];
        $datos["approved"]=$valor["approved"];
